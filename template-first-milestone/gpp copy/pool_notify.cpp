...
pool_notify_Execute (IN Uint32 numIterations, Uint8 processorId)
{
	...
	std::cout << "Pool notification time = " << pnTimer.getMicroSecs() << " us" << std::endl;
                  ...
                  IN Char8 * videoName)
{
    ...
    Uint8      processorId  = 0 ;
    int tryBufferSize;
    tryBufferSize = frame.cols * frame.rows;
    ...
    if (   (dspExecutable != NULL)) {
        ...
        if (DSP_SUCCEEDED (status)) {
            // storeMatrixToBuffer(frame, dataBuffer);
