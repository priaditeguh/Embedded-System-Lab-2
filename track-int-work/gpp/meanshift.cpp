...
void MeanShift::pdf_representation0(const cv::Mat &frame, const cv::Rect &rect, int normalized_C, cv::Mat kernel, cv::Mat &pdf_model)
{
    ...
    for(int i=0;i<rect.height;i++)
    {
        ...
        for(int j=0;j<rect.width;j++)
        {
            ...
            // cout << curr_pixel_value[0] << " " << bin_value << " " << pdf_model.at<int>(0,bin_value) << endl;
            clo_index++;
        }
        row_index++;
    }
...
cv::Rect MeanShift::track(const cv::Mat &next_frame, int normalized_C, cv::Mat kernel)
{
    ...
    for(int iter=0;iter<cfg.MaxIter;iter++)
    {
        ...
        cv::Mat weight = CalWeight_split(next_frame,target_model,target_candidate,target_Region);
